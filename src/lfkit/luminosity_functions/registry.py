...
def _discover_models_package(
    ...
    for module_info in pkgutil.iter_modules(models_pkg.__path__):
        ...
        module = importlib.import_module(
            f"{models_pkg.__name__}.{module_info.name}"
        )
...
def _discover_conditional_models(
    ...
    _register_module_lf_models(
        ...
        from_m_models=None,
        name_transform=_conditional_public_name,
    )
...
def _conditional_public_name(name: str) -> str:
    """Return the public model name for a conditional LF function."""
    public_name = name
